...
def run_god(pan, ms, gt, index, model_name, pareto_data, model_idx):
    print(f"[HWViT + NSGA 2] Index {index} | Model: {model_name}")

    pan_target, ms_target, head_ch = get_model_config(pareto_data, model_name)
    model_path = os.path.join(MODEL_DIR, model_name)

    base_dir = setup_dirs("hwvit_nsga_2")
    ...
    write_metrics("hwvit_nsga_2", index, metrics)
...
def plot_metrics():
    ...
    for idx in indices:
        dir_brovey = os.path.join("plots_hwvit_nsga_2_vs_brovey", str(idx))
        dir_ihs = os.path.join("plots_hwvit_nsga_2_vs_ihs", str(idx))
        ...
        for metric in metrics_list:
            ...
            plt.title(f"{metric} ({direction}) (HWViT + NSGA 2 vs Brovey) - Index {idx}")
            plt.xlabel("HWViT + NSGA 2 Models")
            plt.ylabel("Value")
            ...
            plt.plot([ihs_norm]*len(god_norm2), color='red')

            plt.title(f"{metric} ({direction}) (HWViT + NSGA 2 vs IHS) - Index {idx}")
            plt.xlabel("HWViT + NSGA 2 Models")
            plt.ylabel("Value")
...
def main():
    ...
    for name in ["hwvit_nsga_2", "brovey", "ihs"]:
