...
def plot_metrics():
    ...
    for metric in metrics_list:
        # GOD vs Brovey
        plt.figure()
        plt.bar(range(len(god_df)), god_df[metric].values)
        brovey_val = brovey_df[metric].mean()
        plt.plot([brovey_val]*len(god_df), color='red')

        plt.title(f"{metric} (GOD vs Brovey)")
        plt.xlabel("GOD Models")
        plt.ylabel(metric)
        plt.figtext(0.5, -0.05, "Line = Brovey", ha="center")
        ...
        plt.figure()
        plt.bar(range(len(god_df)), god_df[metric].values)
        ihs_val = ihs_df[metric].mean()
        plt.plot([ihs_val]*len(god_df), color='green')

        plt.title(f"{metric} (GOD vs IHS)")
        plt.xlabel("GOD Models")
        plt.ylabel(metric)
        plt.figtext(0.5, -0.05, "Line = IHS", ha="center")
