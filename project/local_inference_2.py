...
def run_god(pan, ms, gt, index, model_name, pareto_data):
    ...
    # store metrics
    metrics_row = {"pipeline": "GOD", "model": model_name, "index": index}
...
def plot_metrics():
    ...
    for metric in metrics_list:
        ...
        brovey_val = df[df["pipeline"] == "BROVEY"][metric].mean()
        ihs_val = df[df["pipeline"] == "IHS"][metric].mean()

        plt.bar(range(len(god_df)), god_df[metric].values)
        plt.plot([brovey_val] * len(god_df))
        plt.plot([ihs_val] * len(god_df))
